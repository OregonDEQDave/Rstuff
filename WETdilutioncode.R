...
#calculate the Acute WET dilution series
  ...
  c<-(ZDIL/2)
  d<-(ZDIL/4)
print(paste("Acute:","100%",a,"NOEC = ",b,c,d,"0"))
#calculate the Chronic WET dilution series
e<-((RDIL+100)/2)
f<-(RDIL)
g<-(RDIL/2)
h<-(RDIL/4)
print(paste("Chronic:","100%",e,"IC25 =",f,g,h,"0"))
i<-((100+a)/2)
j<-((g/2)
k<-((RDIL+ZDIL)/2)
...
print(paste("Dual Endpoint:","100%",i,ZDIL,k,RDIL,h,0))
    AcuteTU <- 100/ZDIL
    ChronicTU <- 100/RDIL
print(paste("Acute toxic units =",AcuteTU))
print(paste("Chronic toxic units =",ChronicTU))
